...
function getRecipient (person) {
  let recipient = Object.assign({}, normalizeContact(person.dsfData.HOV), { 'SPES-KD': person.dsfData.HOV['SPES-KD'] }, { originalRecipient: normalizeContact(person.dsfData.HOV) })
  if (person.isUnderAge) {
    const guardians = filterGuardians(person.dsfData)
    if (guardians.length > 0) {
      recipient = Object.assign({}, normalizeContact(guardians[0]), { 'SPES-KD': guardians[0]['SPES-KD'] }, { originalRecipient: recipient.originalRecipient })
    } else {
